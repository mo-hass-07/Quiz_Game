...
async function loadQuestionDetail(questionid) {
  ...
  try {
    const q = await apiFetch(`${CONFIG.ROUTES.QUESTIONS}/${questionid}`);
    const currentUserId = getCurrentUserId();
    ...
    container.innerHTML = `
      ...
      <article class="question-card question-detail">
        ...
        <p class="question-meta">by ${q.name || "Unknown"}</p>
    ...
    if (isOwner) {
      document.getElementById("detail-edit-btn").addEventListener("click", () => showQuestionForm(questionid));
      document.getElementById("detail-delete-btn").addEventListener("click", () => deleteQuestion(questionid));
    }
  } catch (err) {
    container.innerHTML = `<p class="error">${err.message}</p>`;
  }
...
// --- Create / Edit ---
async function showQuestionForm(questionid) {
  const container = document.getElementById("questions-container");
  ...
  if (isEdit) {
    try {
      q = await apiFetch(`${CONFIG.ROUTES.QUESTIONS}/${questionid}`);
    } catch (err) {
      container.innerHTML = `<p class="error">${err.message}</p>`;
      return;
    }
  ...
  document.getElementById("question-form").addEventListener("submit", async (e) => {
    ...
    try {
      if (isEdit) {
        await apiFetch(`${CONFIG.ROUTES.QUESTIONS}/${questionid}`, { method: "PUT", body });
      } else {
...
async function playQuestion(questionid) {
  ...
  try {
    ...
    document.getElementById("play-form").addEventListener("submit", async (e) => {
      ...
      try {
        const result = await apiFetch(`${CONFIG.ROUTES.QUESTIONS}/${questionid}/attempt`, {
          method: "POST",
...
async function deleteQuestion(questionid) {
  ...
  try {
    await apiFetch(`${CONFIG.ROUTES.QUESTIONS}/${questionid}`, { method: "DELETE" });
    loadQuestions();
  } catch (err) {
    alert(err.message);
  }
